...
function checkGuess() {
    ...
    if (userGuess > 0 && userGuess < range) {
        ...
        if (userGuess === randomNumber) {
            ...
            lastResult.textContent = 'Попытки закончились :(';
            guessGen.setAttribute('disabled','disabled');
            // coldOrHot.textContent = '';
            setGameOver();
        }
...
function resetGame() {
    guessCount = 1;
    guessGen.removeAttribute('disabled');
    guesses.textContent = '';
    lastResult.textContent = '';
    guess.value = '';
    guessGen.focus();
    lastResult.style.backgroundColor = '$accent-light';
